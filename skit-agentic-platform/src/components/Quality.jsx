          ...
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            >
              ...
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-2xl font-bold text-gray-900">$64,500</span>
                <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700">43 caught today</span>
              </div>
              <div className="text-[13px] text-gray-500">Estimated exposure avoided</div>
            </button>
            ...
            >
              ...
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-2xl font-bold text-gray-900">$12,300</span>
                <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700">leakage</span>
              </div>
              <div className="text-[13px] text-gray-500">Across bot, human & promise breach</div>
            </button>
            ...
            >
              ...
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-2xl font-bold text-gray-900">99.86%</span>
                <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700">above target</span>
              </div>
              <div className="text-[13px] text-gray-500">~$8,200 potentially affected</div>
            </button>
          </div>
              ...
              <div className="grid grid-cols-4 gap-3 mb-5">
                ...
                <div className="rounded-xl p-3" style={{ background: '#f0f8f6' }}>
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-orange-50 text-orange-700">FDCPA</span>
                  </div>
                  <div className="text-[11px] text-gray-400 mt-0.5">All resolved</div>
                </div>
                <div className="rounded-xl p-3" style={{ background: '#f0f8f6' }}>
                  <div className="flex items-baseline gap-2">
                    <span className="text-lg font-bold text-gray-900">1</span>
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700">TCPA</span>
                  </div>
                  <div className="text-[11px] text-gray-400 mt-0.5">All resolved</div>
                </div>
              </div>
          ...
          {expandedSection === 'lost' && (
            <div className="card p-5 mb-4 animate-fadeIn">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-baseline gap-2"><span className="text-2xl font-bold text-gray-900">$12,300</span><span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700">total</span></div>
              </div>

              <div className="grid grid-cols-4 gap-3 mb-5">
                {lostBreakdown.map((item, i) => (
                  <div key={i} className="rounded-xl p-3" style={{ background: '#f0f8f6' }}>
                    <div className="text-lg font-bold text-gray-900" style={{ fontVariantNumeric: 'tabular-nums' }}>${item.amount.toLocaleString()}</div>
                    ...
                    <div className="mt-2 w-full h-1.5 rounded-full" style={{ background: '#d4eae5' }}>
                      <div className="h-1.5 rounded-full" style={{ width: `${item.pct}%`, background: '#2196af' }} />
                    </div>
                  </div>
                ))}
              </div>

              <div className="rounded-xl p-4 grid grid-cols-2 gap-4" style={{ background: '#f0f8f6', border: '1px solid #d4eae5' }}>
                <div>
                  <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Bot Abandonment Rate</div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-lg font-bold text-gray-900">22%</span>
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700">high</span>
                  </div>
                  <div className="text-[11px] text-gray-500">480 conversations, 106 abandoned</div>
                </div>
                <div>
                  <div className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Below Threshold</div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-lg font-bold text-gray-900">3</span>
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700">agents</span>
                  </div>
                ...
                {[
                  ...
                  <div key={i} className="rounded-xl p-3" style={{ background: '#f0f8f6' }}>
                    <div className="flex items-baseline gap-2">
                      ...
                      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${m.pct >= 98 ? 'bg-emerald-50 text-emerald-700' : m.pct >= 96 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>
                        {m.pct >= 98 ? 'on target' : m.pct >= 96 ? 'near' : 'below'}
                      </span>
                    </div>
                    <div className="text-[12px] font-semibold text-gray-700 mt-1 mb-0.5">{m.label}</div>
                    <div className="text-[11px] text-gray-400">{m.bench}</div>
                    <div className="mt-2 w-full h-1.5 rounded-full" style={{ background: '#d4eae5' }}>
                    ...
                    {scoreboard.map((a, i) => (
                      <tr key={i} style={{ borderTop: '1px solid #ecf6f3' }}>
                        ...
                        <td className="py-2.5 px-3 text-right">
                          <span className="font-bold text-gray-900" style={{ fontVariantNumeric: 'tabular-nums' }}>{a.score} / 100</span>
                        </td>
                        <td className="py-2.5 px-3">
                          {a.trend === 'up' && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700">↑ up</span>}
                          {a.trend === 'down' && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-red-50 text-red-700">↓ down</span>}
                          {a.trend === 'stable' && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500">→ flat</span>}
                        </td>
